...
export default function Projects() {
  ...
  e: React.MouseEvent<HTMLDivElement>
      ...
      const x = Math.min(
        Math.max(rawX, cursorWidth / 2 + padding),
        rect.width - cursorWidth / 2 - padding
      );

      const y = Math.min(
        Math.max(rawY, cursorHeight / 2 + padding),
        rect.height - cursorHeight / 2 - padding
      );
      ...
      rafRef.current = requestAnimationFrame(() => {
        ...
        cursorRef.current.style.transform =
          `translate3d(${x - cursorWidth / 2}px, ${y - cursorHeight / 2}px, 0)`;
      });
    };

  const headerContainerVariants =
    isMobile === true
      ? mobileHeaderContainerVariants
      : desktopHeaderContainerVariants;

  const headerItemVariants =
    isMobile === true
      ? mobileHeaderItemVariants
      : desktopHeaderItemVariants;
    ...
    <>
      ...
      >
        ...
        >
          ...
          >
            Criando experiências imersivas com foco em
            desempenho, qualidade de código e solução.
          </motion.p>
        </motion.div>
                  ...
                  {isDesktop && (
                    <AnimatePresence>
                      {activeCardId ===
                        ...
                        >
                          <span className="proj-cursor-eyes">
                            🔍
                          </span>

                          <span className="proj-cursor-text">
                            Ver mais
                          </span>
                        </motion.div>
                      )}
